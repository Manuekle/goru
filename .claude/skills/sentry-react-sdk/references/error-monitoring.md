...
```jsx
// 1. Static element
<Sentry.ErrorBoundary fallback={<p className="sm:text-sm text-xs">Something went wrong. Please refresh.</p>}>
  <Dashboard />
</Sentry.ErrorBoundary>
...
<Sentry.ErrorBoundary
  fallback={({ error, componentStack, resetError }) => (
    <div className="error-state">
      ...
      <p className="sm:text-sm text-xs"><strong>Error:</strong> {error.message}</p>
      <details>
...
<Sentry.ErrorBoundary
  ...
  fallback={({ resetError }) => (
    <div>
      <p className="sm:text-sm text-xs">Payment failed to load.</p>
...
<Sentry.ErrorBoundary
  ...
  fallback={
    <p className="sm:text-sm text-xs">
      We've logged this issue and are working on a fix.
    </p>
  }
>
  <Dashboard />
...
const SafeDashboard = Sentry.withErrorBoundary(Dashboard, {
  fallback: <p className="sm:text-sm text-xs">Dashboard failed to load.</p>,
});
...
const SafeCheckout = Sentry.withErrorBoundary(CheckoutFlow, {
  fallback: ({ error, resetError }) => (
    <div>
      <p className="sm:text-sm text-xs">Checkout error: {error.message}</p>
      <button onClick={resetError}>Retry</button>
    </div>
  ),
  onError: (error, componentStack, eventId) => {
...
class CustomBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError)
      return <p className="sm:text-sm text-xs">Something went wrong.</p>;
...
function UserProfile({ userId }) {
  ...
  if (error)
    return <p className="sm:text-sm text-xs">Failed to load profile.</p>;
...
function PaymentForm() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {status === "error" && (
        <p className="sm:text-sm text-xs">Payment failed. Please try again.</p>
      )}
    </form>
  );
}
