...
export function SentryRouteErrorBoundary() {
  ...
  return (
    <div role="alert">
      ...
      <p className="sm:text-sm text-xs">{error?.message ?? "An unexpected error occurred."}</p>
    </div>
  );
}
...
ReactDOM.createRoot(document.getElementById("root")!).render(
  <StrictMode>
    <Sentry.ErrorBoundary fallback={<p className="sm:text-sm text-xs">An error has occurred</p>}>
      <RouterProvider router={router} />
